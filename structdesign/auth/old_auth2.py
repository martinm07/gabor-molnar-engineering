...
from structdesign._DEPR_models import (
    DRequestStamp,
    DUser,
    DUserBackupFactor,
    DUserSecret,
...
from structdesign.helper import cors_enabled, country_code_to_prefix, send_email_info
...
def is_email_valid(email, fast=False):
    if not fast:
        return (
            validate_email(email_address=email) is not False
        )  # `not (... == False)` to evaluate ambiguous results as True
    else:
...
def is_valid_email():
    ...
    if is_valid:  # We don't want legitimate users having to wait for validation again in "/set_info"
        session["register_valid_emails"] = session.get("register_valid_emails", []) + [
            email
        ]
...
def set_info():
    ...
    if (type_ == "email") and (
        (info in session.get("register_valid_emails", [])) or is_email_valid(info)
    ):
        user.email = info
...
def true_rand():
    return sha256("".join([str(item) for item in list(os.urandom(5))]))
...
def send_token():
    ...
    can_send, attempts, last_attempt = (
        can_sendtoken_info()
    )  # last_attempt -> seconds since last attempt
    ...
    return_data = returnMsg() | {
        "info": info,
        "infoType": session["register_info_type"],
        "timeout": max(
            0, token_penalty(attempts + (1 if last_attempt == 0 else 0)) - last_attempt
        ),
    }
...
def wait_until_resend_ready():
    ...
    print(
        "Going to sleep for "
        + str(token_penalty(num_attempts) - last_attempt)
        + " seconds."
    )
...
def recovery_option_isinuse():
    ...
    return jsonify(
        sum([factor.data == info for factor in user.backup_factors]) > 0
        or info == possession
    )
...
def get_country_code():
    ...
    if user and user.phone_number:
        ...
        phones = [
            factor.data for factor in user.backup_factors if factor.method == "phone"
        ]
        if phones != []:
...
def new_recovery_option_checks(data, type_):
    ...
    if type_ == "email":
        ...
        if g.phone.phone_number in [
            factor_.data for factor_ in user.backup_factors
        ] or (g.phone.phone_number == user.phone_number):
            return {"message": "Already in use by user."}, 400
    return {}, 200
...
def get_reg_state():
    ...
    recovery = [
        {"info": factor.data, "type": factor.method, "id": factor.id}
        for factor in user.backup_factors
    ]
    return make_dict(
        name,
        possession_type,
