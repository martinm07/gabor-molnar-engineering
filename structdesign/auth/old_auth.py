...
from structdesign.helper import cors_enabled, host_is_local
from structdesign.models import User, UserBackupFactor, UserSecret

bp = Blueprint("old_auth", __name__, url_prefix="/legacy")
...
def send_voice(
    text="Hello world, this was generated on a Python Flask server.",
    to="+353877211985",
    from_="+19706458553",
):
    if host_is_local(request.url_root):
        url = "http://legoboy7.pythonanywhere.com/auth/twiml_voice/" + text.replace(
            " ", "%20"
        )
    else:
        url = urljoin(request.url_root, url_for("auth.twiml_voice", text=text))

    client.calls.create(url=url, to=to, from_=from_)
    return "Call sent! " + url


def token_penalty(attempt_num):
    retry_timeouts = {1: 30, 2: 40, 3: 60, 4: 90, 5: 120}
    return retry_timeouts.get(attempt_num, 600)


def token_attempt(method):
    print(
        session.get("attempt_num"),
        session.get("last_attempt"),
        session.get("tokentimeoffset"),
    )
    no_penalty_methods = ["email"]
    last_attempt = session.get("last_attempt", 0.0)
    attempt_num = session.get("attempt_num", 0)
    if (
        method not in no_penalty_methods
        and time.time() - token_penalty(attempt_num) <= last_attempt
    ):
        return False
...
def compute_token():
    ...
    token = str(int(sha256(secret.secret + curtime + method + data), 16))[
        -7:
    ]  # int(..., 16) converts hex to decimal
    print(curtime, token)
    return token
...
def register_details():
    if request.method == "POST":
        ...
        if name_taken["is_taken"]:
            return {
                "message": f'[username_taken] User of name "{data["username"]}" already exists in database.'
            }, 400
        email_taken = json.loads(is_email_taken(data["email"]).data.decode("utf-8"))
        if email_taken["is_taken"]:
            return {
                "message": f'[email_taken] Email "{data["email"]}" already exists in database.'
            }, 400
        ...
        try:
            ...
            if session.get("user_id") and session.get("registering"):
                ...
                if (new_user.email != email and new_user.twofa_method == "email") or (
                    ...
                    and new_user.twofa_method in ["sms", "voice"]
                ):
    ...
    if request.method == "POST":
        try:
            ...
            if not token_attempt("sms"):
                return {
                    "message": "Buffer time still active, thus not sending token."
                }, 403
            ...
            return {
                "buffer_penalty": token_penalty(session.get("attempt_num")),
                "twofa_data": number.national_format,
                "twofa_method": "sms",
            }
        except Exception as err:
            return {"message": err.__str__()}, 500
...
def send_token_voice():
    if request.method == "POST":
        try:
            ...
            send_voice(
                "Here is your verification token: " + code, to=session["twofa_data"]
            )
...
def register_get_secure():
    ...
    if is_token_sent:
        retry_buffer = token_penalty(session["attempt_num"]) - ceil(
            time.time() - session["last_attempt"]
        )
    else:
        retry_buffer = 0
    ...
    return {
        "is_password_made": is_password_made,
        "is_token_sent": is_token_sent,
        "is_2fa_made": is_2fa_made,
...
def register_recovery():
    ...
    try:
        ...
        if recover_type == "number":
            ...
            if not number.valid:
                return {"message": "[invalid_phone] Invalid phone number."}, 400
            number_inuse = json.loads(
                is_number_inuse(data["phonenumber"]).data.decode("utf-8")
            )
            if number_inuse["is_inuse"]:
                return {
                    "message": "[number_inuse] Phone number has already been registered for normal 2FA."
                }, 400
            ...
            if email_inuse["is_inuse"]:
                return {
                    "message": "[email_inuse] Email has already been registered for normal 2FA."
                }, 400
...
def register_get_recovery():
    ...
    return jsonify(
        [
            {"data": opt.data, "method": opt.method, "id": opt.id}
