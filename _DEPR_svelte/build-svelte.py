...
if not os.environ.get("FLASK_APP"):
    raise Exception(
        'Environment variable "FLASK_APP" is not set. Please set it to relevant value to run the Flask app (e.g. "flask-app".'
    )
...
def fixPath(path: str, source_dirpath: str = "intro/js/"):
    ...
    if "/assets/" not in path:
        warn(
            ' "/assets/" not part of this absolute file path: "'
            + path
            + '". Behaviour uncertain.'
        )
...
def jsUpdateFile(src: str, source_dirpath: str = "intro/js/"):
    ...
    js_regex = re.compile(
        r"(\").*?(?<!\\)\1"
    )  # Note here we assume that the built js files will never use
...
for root, dirs, files in os.walk("dist/"):
    ...
    def get_etag(filepath):
        return (
            "-".join((filepath.split("/")[-1]).split("-")[:-1])
            + f".{filepath.split('.')[-1]}"
        )

    def check_ext(filename, ext):
        filename.split(".")[-1] == ext

    # get_etag = (
    #     lambda filepath: "-".join((filepath.split("/")[-1]).split("-")[:-1])
    #     + f".{filepath.split('.')[-1]}"
    # )
    ...
    for file in files:
        ...
        if os.path.exists(future_dirpath):
            for old_filename in os.listdir(future_dirpath):
                old_etag = (
                    get_etag(old_filename)
                    if not check_ext(old_filename, "html")
                    else old_filename
                )
