<script lang="ts">
  ...
  function loadData(data: IncomingData[]) {
    ...
    inputTags.splice(
      0,
      inputTags.length,
      ...Array.from(Array(final.length), () => new Object()),
    );
...
<svelte:document
  onkeydown={(e) => {
    ...
    if (
      e.key === "Enter" &&
      ed.activeI !== -1 &&
      document.activeElement?.closest(".tag-btn")
    ) {
    ...
    if (ed.activeI !== -1 && (e.key === "ArrowDown" || e.key === "ArrowUp"))
      e.preventDefault();
  }}
/>
...
<div class="grid grid-rows-4 h-screen">
  <div
    class="flex flex-col items-end bg-background-50 row-span-3 overflow-y-scroll"
  >
    <div class="sticky top-5" id="add-tag-container">
      <button
        ...
        ><span class="text-green-600 text-6xl/12 mr-2 mb-0.5">+</span><span
          class="font-bold text-lg">Add tag</span
        ></button
      >
    </div>

    <div class="text-4xl font-serif text-center text-rock-700 mt-5 w-full">
      Manage Guidance Document Tags
    </div>
    <div class="max-w-3xl mx-auto h-fit my-5 px-4">
      {#each tags as tag, i}
        {const color = $derived(
          Color(
            convertAccent((!tag.accentDisabled && tag.accent) || "#e7e8e3"),
          ),
        )}
        ...
        >
          <div class="text-xl text-rock-700 font-mono font-bold">
            {tag.name}
          </div>
          <div class="text-rock-700 mb-1.5">{@html tag.description}</div>
          <div class="text-sm text-rock-600 italic">
            Used to tag {tag.documentTitles.length} document{tag.documentTitles
              .length === 1
              ? ""
              : "s"}
          </div>
    ...
    {#if ed.changes}
      ...
      >
        <button
          ...
          ><span class="text-green-100 text-4xl my-2 mr-2"
            ><FloppyDiskIcon /></span
  ...
  <div
    class="flex justify-center border-t-2 p-4 border-rock-300 bg-background h-fit min-h-full"
  >
    <BottomTray />
  </div>
</div>
...
<style>
  ...
  #save-changes-container {
    right: max(
      0px,
      calc(
        50% - var(--tags-max-container-width) / 2 - var(--btn-width) -
          var(--padding-x)
      )
    );
