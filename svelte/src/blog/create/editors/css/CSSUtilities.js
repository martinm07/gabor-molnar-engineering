/*******************************************************************************
 ...
 -------------------------------------------------------------------------------
 Copyright (c) 2010 brothercake                      http://www.brothercake.com/
 Info/Docs       http://www.brothercake.com/site/resources/scripts/cssutilities/
 lGPL License               http://www.gnu.org/licenses/lgpl-3.0-standalone.html
 -------------------------------------------------------------------------------
 Credits and thanks:
 -------------------------------------------------------------------------------
...
(function () {
  ...
  Z.init = function (X5) {
    ...
    Z._N5 =
      (L1 && page.documentElement.namespaceURI != U0) ||
      (!L1 && typeof page.xmlVersion != N0 && page.xmlVersion != U0);
    Z._Y6 = f30();
    Z._S1 = U0;
    delete Z._S1;
    Z._T1 = U0;
      ...
      for (var i = 0; i < rules.length; i++) {
        for (var j in rules[i].properties) {
          if (
            ...
            rules[i].properties[j].status != X0
          ) {
            continue;
          }
    ...
    function E7() {
      ...
      for (var i = 0; i < A4.length; i++) {
        f10(
          A4[i],
          A4[i].nodeType == 7
            ? A4[i].target.toLowerCase()
            : A4[i].nodeName.toLowerCase(),
        );
      }
      function f1(Y5) {
  ...
  function f9() {
    ...
    for (var i = 0; i < V4.length; i++) {
      if (
        V4[i].nodeType == 7 &&
        V4[i].target.toLowerCase() == "xml-stylesheet"
      ) {
        A4.push(V4[i]);
      }
    }
    ...
    for (var i = 0; i < E2.length; i++) {
      if (
        (/link/i.test(E2[i].nodeName) &&
          /stylesheet/i.test(E2[i].getAttribute("rel"))) ||
        /style/i.test(E2[i].nodeName)
      ) {
        A4.push(E2[i]);
      }
    }
  ...
  function f13(J4, J6) {
    ...
    J4 = J4.replace(/[\r\n]/g, "");
    J4 = J4.replace(/@(charset|namespace)[^;]+;/gim, "");
    if (typeof J6 != N0 && J6 == S0) {
      J4 = J4.replace(/@import[^;]+;/gim, "");
    } else if (/@import[^;]+;/i.test(J4)) {
      var V1 = J4.replace(/(@import[^;]+;)/gim, "{SPLIT}$1{SPLIT}").split(
        "{SPLIT}",
      );
    ...
    J4 = J4.replace(/@(font\-face|page)[^\}]+\}/gim, "");
    J4 = J4.replace(/(<\!\-\-)|(\-\->)|(<\!\[CDATA\[)|(\]\]>)/gm, "");
    return f42(J4);
  }
  function f14(V1) {
    var T5 = V1.text.match(/(content:[^;]+;)/gim);
    if (T5) {
      ...
      if (typeof O2[i].ownerNode != N0) {
        if (G1 && /xml\-stylesheet/.test(O2[i].ownerNode.nodeName)) {
          ...
          try {
            f16(
              O2[i],
              O2[i].ownerNode.nodeName.toLowerCase(),
              O2[i].media.length > 0 ? f42(O2[i].media.mediaText) : B1,
            );
          } catch (err) {
            Z._T1.push({
              ssid: O2[i].__ssid,
        ...
        f18(
          O2[i],
          O2[i].owningElement.nodeName.toLowerCase(),
          f42(O2[i].media) != "" ? f42(O2[i].media) : B1,
        );
      }
    }
    Z._T1.sort(function (a, b) {
    ...
    for (var i = 0; i < rules.length; i++) {
      ...
      if (rule.type == 1) {
        ...
        for (var c = 0; c < J4.length; c++) {
          ...
          if (
            rule.style.getPropertyPriority(f42(J4[c][0])) == "important" &&
            !L0.test(J4[c][1])
          ) {
            J4[c][1] += " !important";
          }
  ...
  function f18(sheet, owner, media, href) {
    ...
    try {
      if (!Q4) {
        ...
        J4 = J4.replace(/@(import|charset|namespace)[^;]+;/gim, "");
        J4 = J4.replace(/@(font\-face|page)[^\}]+\}/gim, "");
        var T5 = J4.match(/(content\s*:\s*[\'\"].*(;|$))/gim);
        if (T5) {
          for (var g = 0; g < T5.length; g++) {
        ...
        if (typeof href == N0) {
          href =
            sheet.href == U0 || sheet.href == "" ? U0 : f47(sheet.href, base);
        }
        var rules = [];
        var V2 = sheet.media;
        if (V2 == "") {
  ...
  function f20(D5, media, F5, S5, rules, B2, F2) {
    ...
    for (var i = 0; i < Z._S1.length; i++) {
      ...
      if (Z._S1[i].owner == "@style" && D5 == F2[0]) {
        ...
        if (S5 == S0) {
          for (
            var altstate = S0, E2 = f33(Z._S1[i].selector), j = 0;
            j < E2.length;
            j++
          ) {
            if (E2[j] == D5) {
        ...
        D4 = f33(
          S5 == S0 ? Z._S1[i].selector.replace(H0, "") : Z._S1[i].selector,
        );
      }
      if (D4.length > 0) {
        for (var j = 0; j < D4.length; j++) {
          if (D4[j] == D5) {
            ...
            for (
              var G2 = ["media", "xmedia", "owner", "ssid", "href"], k = 0;
              k < G2.length;
              k++
            ) {
  ...
  function f22(rules, U5) {
    for (var i = 0; i < rules.length; i++) {
      rules[i].properties = f27(
        rules[i].css,
        {},
        typeof rules[i].inheritance != N0 && rules[i].inheritance.length > 0,
      );
      rules[i].T6 = {};
      for (var j in rules[i].properties) {
        ...
        if (rules[i].altstate == S0) {
          ...
          for (var j = 0; j < i; j++) {
            for (var jp in rules[j].T6) {
              if (
                !rules[j].T6.hasOwnProperty(jp) ||
                rules[j].T6[jp].status != X0
              ) {
                continue;
              }
              for (var ip in rules[i].T6) {
                if (
                  !rules[i].T6.hasOwnProperty(ip) ||
                  rules[i].T6[ip].status != X0
                ) {
                  continue;
                }
                if (jp == ip) {
        ...
        for (var j in rules[i].T6) {
          ...
          if (L0.test(rules[i].T6[j].value)) {
            ...
            for (var x = i + 1; x < rules.length; x++) {
              if (
                typeof rules[x].T6[j] != N0 &&
                rules[x].T6[j].status == Y0 &&
                L0.test(rules[x].T6[j].value)
              ) {
                E3 = x;
              }
            }
            for (var x = i; x < rules.length; x++) {
              if (x == E3) {
                ...
                for (var y = 0; y < x; y++) {
                  if (
                    typeof rules[y].T6[j] != N0 &&
                    rules[y].T6[j].status == X0
                  ) {
          ...
          if (typeof D0[j] != N0) {
            for (var x = 0; x <= i; x++) {
              for (var p = 0; p < D0[j].length; p++) {
                var R1 = D0[j][p];
                if (
                  typeof rules[x].T6[R1] != N0 &&
                  rules[x].T6[R1].status == X0
                ) {
                  if (x == i) {
                    var n = 0;
  ...
  function f23(selector) {
    ...
    var H3 = F3.replace(H0, "")
      .replace(G0, "")
      .replace(/(:not)/gi, "")
      .replace(/(^|\()([_a-z0-9-\.\\]+\|)/gi, "$1")
      .replace(J0, "")
      .replace(K0, "");
    var C2 = H3.match(/([_a-z0-9-:\\]+)/gi);
    if (C2) {
      G3[3] += C2.length;
    }
    var C2 = F3.match(G0);
  ...
  function f24(Q3) {
    ...
    var P3 = Q3.match(/([#\.][a-z]+[_a-z0-9-:\\]*)/gi);
    if (P3) {
  ...
  function f27(J4, properties, B2) {
    ...
    for (var i = 0; i < J4.length; i++) {
      ...
      if (!B2 || (B2 && typeof C0[O3] != N0)) {
        if (
          typeof properties[O3] == N0 ||
          !L0.test(properties[O3].property) ||
          properties[O3].B2 == S0 ||
          L0.test(C2[1])
        ) {
  ...
  function f29(media, rule) {
    ...
    for (var j = 0; j < media.length; j++) {
      if (
        typeof D2[media[j]] != N0 ||
        (typeof D2[D1] == N0 && media[j] == B1) ||
        (typeof D2[B1] != N0 && media[j] != D1)
      ) {
        C2 = S0;
        break;
      }
  ...
  function f30() {
    ...
    try {
      ...
      for (var i = 0; i < D2.length; i++) {
        ...
        if (
          (K1 && X6.currentStyle.display == "block") ||
          (!K1 &&
            page.defaultView
              .getComputedStyle(X6, "")
  ...
  function f31() {
    for (var U3 = [], i = 0; i < Z._T1.length; i++) {
      var stylesheet =
        mode == W0
          ? Z._T1[i].stylenode == T0 || Z._T1[i].stylenode == U0
            ? U0
            : H1 || I1 || J1
              ? Z._T1[i].stylenode
              : Z._T1[i].stylenode[
                  typeof Z._T1[i].stylenode.styleSheet != N0
                    ? "styleSheet"
                    : "sheet"
                ]
          : Z._T1[i].stylesheet;
