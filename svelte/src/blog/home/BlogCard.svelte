...
<div
  ...
  {style}"
  class="h-fit card-root max-w-96 relative flex items-center mt-[--top-height] has-[a:hover]:-mt-[calc(var(--push-up)-var(--top-height))] transition-all"
>
  <a
    href="/read/article"
    class="peer group block cursor-pointer h-full w-[calc(100%-2*var(--side-halfwidth))] bg-rock-50 rounded-bl-[--side-halfwidth] border-2 border-r-0 border-[--border-color] p-4 transition-all hover:pb-[calc(1rem+var(--push-up))] z-20 relative"
  >
    <h1
    ...
    <svg
      ...
      viewBox="-4 -3 8 8"
      class="absolute bottom-0 group-hover:bottom-(--push-up) group-hover:h-[calc(100%-var(--push-up))] transition-all w-full h-full opacity-15"
    >
      <path
        d={svgPath}
  ...
  <div
    class="absolute right-0 top-0 h-full w-[calc(2*var(--side-halfwidth))] bg-[rgb(var(--accent-light)/1)] rounded-b-(--side-halfwidth) border-2 border-t-0 border-(--border-color) peer-hover:h-[calc(100%-var(--push-down))] peer-hover:top-(--push-down) transition-all z-0"
  ></div>
  <div
    class="absolute w-[calc(2*var(--side-halfwidth))] h-(--side-halfwidth) bg-rock-50 bottom-0 right-0 -z-10 rounded-br-(--side-halfwidth) border-b-2 border-(--border-color) transition-all"
  ></div>
  <div
    class="absolute top-0 right-0 bg-[rgb(var(--accent-light)/1)] w-[calc(100%-2*var(--side-halfwidth))] -translate-y-full h-(--top-height) border-2 border-b-0 border-(--border-color) peer-hover:top-(--push-down) transition-all z-10"
  ></div>
  <div
    class="absolute z-30 w-0.5 bg-(--border-color) h-(--push-down) right-[calc(2*var(--side-halfwidth))] top-0 translate-x-full"
  ></div>
</div>
